...
public:
        ...
        int z)
    {
        ...
        if (find(x, y, z)) {
            cout << "Point already exist in the tree: "<<x<<", "<<y<<", "<<z << endl;
            return;
        }
...
int main()
{
    ...
    inputPoints.push_back(Point(maxX,maxY,maxZ));

    int inputX, inputY, inputZ;
    
    while(!readd.eof())
    {
        readd >> inputX >> inputY >> inputZ;
        inputPoints.push_back(Point(inputX, inputY, inputZ));
        if (inputX > maxX)
        {
            maxX = inputX;
        }
